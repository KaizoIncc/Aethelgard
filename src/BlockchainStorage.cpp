...
#include <zlib.h> // Para compresión opcional
...
bool BlockchainStorage::loadBlock(uint64_t index, Block& block) {
    lock_guard<mutex> lock(storageMutex);
    ...
    try {
        ...
        cerr << "Error loading block: " << e.what() << endl;
        return false;
    }
}
...
bool BlockchainStorage::loadLastBlock(Block& block) {
    ...
    return loadBlock(latestIndex, block);
}
